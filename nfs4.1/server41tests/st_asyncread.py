...
def testAsyncRead(t, env):
    ...
    reqId = getReqId()
    async_res = sess1.compound([op.putfh(fh), op.async_read(reqId,stateid,0,10)])
    check(async_res)
    asyre.wait() 
...
def testMultipleAsyncReads(t,env):
    ...
    sess1.client.cb_pre_hook(OP_CB_ASYNC_READ, pre_hook)
    res1 = sess1.compound([op.putfh(fh), op.async_read(reqId,stateid,0,10)])
    check(res1)
    reqId=getReqId()
    reqIdHookMap[reqId] = receivedHk2
    res2 = sess1.compound([op.putfh(fh), op.async_read(reqId,stateid,7,15)])
    check(res2)
    asyre.wait() 
...
def testEOFInAsyncRead(t,env):
    ...
    sess1.client.cb_pre_hook(OP_CB_ASYNC_READ, pre_hook)
    reqId=getReqId()
    res1 = sess1.compound([op.putfh(fh), op.async_read(reqId,stateid,0,1000)])
...
def testAsyncReadInDifferentSessions(t,env):
    ...
    sess2.client.cb_pre_hook(OP_CB_ASYNC_READ, pre_hook)
    reqId=getReqId()
    res1 = sess2.compound([op.putfh(fh), op.async_read(reqId,stateid,4,8)])
